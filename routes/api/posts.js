...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (!post) {
      return res.status(404).json({ msg: 'No post for this id' })
    }
    res.json(post)
  } catch (err) {
    console.error(err.message)
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    if (post.user.toString() !== req.user.id) {
      return res
        .status(401)
        .json({ msg: 'User not authorized for deleting this post' })
    }
    await post.remove()
...
// desc   : Like post of following Id
// access : Private
...
// desc   : Unlike post of following Id
// access : Private
